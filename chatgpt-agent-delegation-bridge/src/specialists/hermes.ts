...
function evidence(payload: unknown) {
  return {
    name: 'hermes-delegation-result.json',
...
export class HermesSpecialist {
  ...
  async inspect(
    ...
    try {
      ...
      return {
        ...
        target: 'HERMES',
        status: 'SUCCEEDED',
        summary: `Hermes completed the requested read-only ${input.kind} inspection.`,
        result: safe,
        warnings: [],
  ...
  async execute(
    ...
    try {
      ...
      return {
        ...
        target: 'HERMES',
        status: 'SUCCEEDED',
        summary: `Hermes completed ${outputs.length} explicitly approved bounded operation(s).`,
        result: safe,
        warnings: [],
        evidence: [evidence(safe)],
        retryable: false,
  ...
  health(): Record<string, unknown> {
    return {
      enabled: this.config.hermes.enabled,
      ready: this.config.hermes.enabled,
  ...
  private async executeOperation(
    workspace: WorkspaceRegistration,
    request: HermesExecuteInput,
    toolId: HermesExecuteInput['operations'][number]['toolId'],
