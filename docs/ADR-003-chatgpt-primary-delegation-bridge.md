...
ChatGPT performs every task it can complete with its own reasoning and native/connected tools. ChatGPT calls the delegation bridge only when it intentionally needs a specialist AI or bounded local capability.
...
ChatGPT project
  ...
  └─ explicit MCP call only when needed
       ├─ Codex AI specialist
       ├─ bounded local executor (not an AI)
       └─ fixed Agents SDK specialist agents
...
A real Hermes AI integration, if later approved and technically available, must be introduced as a separate specialist tool that calls the actual Hermes service/model. Local file, PowerShell, Docker, or Scheduled Task operations must never borrow the Hermes name.
