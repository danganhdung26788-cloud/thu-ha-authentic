...
class ProductionHardeningContractTests(unittest.TestCase):
    ...
    def test_conversation_runtime_uses_native_skill_without_training_history_injection(self):
        adapter = self.read("conversation_runtime_processor.py")
        core = self.read("conversation_runtime_core.py")
        self.assertIn("conversation_runtime_core", adapter)
        self.assertIn("def build_conversation_prompt", adapter)
        self.assertIn("/thu-ha-cosmetics", adapter)
        self.assertIn("Không chèn hoặc đọc lịch sử training thô", adapter)
        self.assertIn("def read_active_training_memory", adapter)
        self.assertIn('return ""', adapter)
        self.assertNotIn("ACTIVE_TRAINING_MEMORY_PATH", adapter)
        self.assertNotIn("TRAINING ĐANG CÓ HIỆU LỰC", adapter)
        self.assertNotIn("MEMORY_PATH", adapter)
        self.assertNotIn("USER_PATH", adapter)
        self.assertIn("def infer_forced_request", core)
        self.assertIn("def fast_product_choice_reply", core)
        self.assertIn("def resolve_product_refs", core)
        self.assertIn("def retrieve_recommendation_candidates", core)
        self.assertIn("def extract_budget_vnd", core)
        self.assertIn("Grounded recommendation cannot be sent without PRODUCT_KEY", core)
        self.assertIn('repo.update_status(row_number, "PROCESSING")', core)
        self.assertNotIn("nói thêm giúp em một chút về điều chị đang cần", core)

    def test_telegram_training_updates_skill_with_snapshot_audit_and_rollback(self):
        helper = self.read("telegram_skill_learning.py")
        training_skill = self.read("skills/thu-ha-training/SKILL.md")
        cosmetics_skill = self.read("skills/thu-ha-cosmetics/SKILL.md")
        installer = self.read("install_telegram_training_chat.ps1")
        self.assertIn("def snapshot", helper)
        self.assertIn("def verify", helper)
        self.assertIn("def abort_pending", helper)
        self.assertIn("def rollback_latest", helper)
        self.assertIn("audit.jsonl", helper)
        self.assertIn("Do not store live price, stock, or promotion values", helper)
        self.assertIn("name: thu-ha-training", training_skill)
        self.assertIn("requires_toolsets: [skills, terminal]", training_skill)
        self.assertIn("skill_manage", training_skill)
        self.assertIn("skill_view", training_skill)
        self.assertIn("telegram_skill_learning snapshot", training_skill)
        self.assertIn("telegram_skill_learning verify", training_skill)
        self.assertIn("telegram_skill_learning abort", training_skill)
        self.assertNotIn("telegram_training_memory apply", training_skill)
        self.assertIn("Progressive disclosure", cosmetics_skill)
        self.assertIn("references/sales-flow.md", cosmetics_skill)
        self.assertIn("Không đọc toàn bộ lịch sử training", cosmetics_skill)
        self.assertIn("THA_REPLY_MODE' -Value 'DRAFT_ONLY", installer)
        self.assertIn("THA_META_AUTO_SEND' -Value 'false", installer)
        self.assertIn("Remove-EnvKey", installer)
        self.assertIn("THA_ACTIVE_TRAINING_MEMORY_PATH", installer)
        self.assertIn("TRAINING_HISTORY_IN_PROMPT=FALSE", installer)
        self.assertIn("LEARNING_MODE=HERMES_SKILL_MANAGE", installer)
