...
$postgresId = (& docker compose --env-file .env -f compose.yml ps -q postgres).Trim()
if (-not $postgresId) { throw 'PostgreSQL container is not running.' }
& docker cp $databaseDump "${postgresId}:/tmp/agent_v2.dump"
if ($LASTEXITCODE -ne 0) { throw 'Copying database dump into container failed.' }
& docker compose --env-file .env -f compose.yml exec -T postgres sh -c 'dropdb -U agent_v2 --if-exists agent_v2 && createdb -U agent_v2 agent_v2 && pg_restore -U agent_v2 -d agent_v2 --clean --if-exists /tmp/agent_v2.dump'
if ($LASTEXITCODE -ne 0) { throw 'PostgreSQL restore failed.' }
& docker compose --env-file .env -f compose.yml exec -T postgres rm -f /tmp/agent_v2.dump

& docker compose --env-file .env -f compose.yml stop minio
...
& docker run --rm -v "${minioVolume}:/data" -v "${mount}:/backup:ro" alpine:3.21 sh -c 'rm -rf /data/* /data/.[!.]* /data/..?* 2>/dev/null || true; tar -xzf /backup/minio-data.tgz -C /data'
if ($LASTEXITCODE -ne 0) { throw 'MinIO restore failed.' }

& docker compose --env-file .env -f compose.yml up -d minio api worker
