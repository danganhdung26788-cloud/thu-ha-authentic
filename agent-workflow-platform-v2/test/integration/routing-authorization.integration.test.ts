...
test('registered tool risk drives deterministic mutation and deep intervention policy', { skip: !integrationEnabled }, async () => {
  resetEnvForTests();
  try {
    // Executor adapters remain TESTING in the production seed until their live
    // contract tests pass. This isolated integration fixture activates only the
    // agents needed to verify grant and risk-policy behavior.
    await getPool().query(
      `UPDATE agent_registry
       SET status = 'ACTIVE', updated_at = now()
       WHERE agent_id IN ('hermes', 'codex')`,
    );
